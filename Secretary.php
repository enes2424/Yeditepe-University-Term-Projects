...
if (isset($_SESSION["Secretary"]))
	foreach	($_SESSION["Secretary"] as $index => $info)
...
$username = $_SESSION["Secretary"][$id]["username"];
echo "<title>".$_SESSION["Secretary"][$id]["name"]."</title>";
...
if (isset($_POST["select_exam"])) {
	if ($_POST["date"] == "")
		...
		if ($num_of_assistant == null)
			...
			$sql .= " WHERE employee.department_id = department.department_id";
			$sql .= " and role = 'Assistant' and employee.department_id = '".$_SESSION["Secretary"][$id]["department_id"]."'";
...
if (isset($_POST["displayScores"])) {
	$sql = "SELECT employee_id FROM employee WHERE role = 'Assistant' and department_id = ".$_SESSION["Secretary"][$id]["department_id"];
	$result = mysqli_query($conn, $sql) or die("Error");
	if (mysqli_num_rows($result) > 0)
		while($row = mysqli_fetch_array($result))
